...
async def cmd_myid(message: Message):
    """Har bir foydalanuvchi o'z Telegram ID'ini bilishi uchun."""
    await message.answer(
        f"🆔 Sizning ID'ingiz: <code>{message.from_user.id}</code>\n\n"
        f"Buni admin qilish uchun Railway'dagi <b>ADMIN_ID</b> "
        f"o'zgaruvchisiga aynan shu raqamni yozing."
    )


@router.message(CommandStart())
async def cmd_start(message: Message):
    from handlers.admin import admin_menu_keyboard as admin_kb
    ...
    if message.from_user.id == ADMIN_ID:
        await message.answer(
            ...
            f"Sizga admin panel ochildi — pastdagi tugmalardan foydalaning.\n"
            f"Yoki istalgan vaqtda <code>/admin</code> deb yozing.",
